...
If the API itself is returning incorrect, incomplete or inaccurate data then an issue needs to be raised **with the API provider** (not Dashy!). You can find the API provider included within the widget docs, or for a full list see the [Privacy Docs](/docs/privacy.md#widgets).
...
- Or setup SSL - [here's a guide](/docs/management.md#ssl-certificates) on doing so
